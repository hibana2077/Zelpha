"""
Ablation and Visualization for Toy Experiments (Zelpha vs components)

This script produces:
- Graph ablation: H-only (RKHS-cosine), Heat-only, Zelpha (H^alpha * Heat^(1-alpha))
- Visualizations:
  1) Adjacency matrices and pairwise differences
  2) Feature signal diffusion extent via heat kernel propagation from seed nodes

Datasets: two moons, circles, swiss roll (basic support; 2D scatter for moons/circles).

Reuses graph builders/utilities from `swiss_roll_experiment.py`.
"""
...
mpl.use("Agg")  # non-interactive backend for safe file saving in headless environments
...
# Reuse utilities
from swiss_roll_experiment import (
    rbf_kernel,
    topk_sparsify,
...
def compute_H(X: np.ndarray, kernel: str = "rbf", gamma: float | None = None) -> np.ndarray:
    """Compute RKHS-cosine matrix H = D^{-1/2} K D^{-1/2}.
    For RBF kernel, K_ii = 1 so H == K.
    """
    if kernel == "rbf":
        if gamma is None:
            # median heuristic on pairwise distances
            # use a standardized space for scale stability
...
def graphs_from_ablation(
    ...
    """Build graphs and intermediate matrices for ablation.

    Returns a dict with keys:
      H_dense (np.ndarray), K_heat_dense (np.ndarray),
      A_H (csr), A_heat (csr), A_zelpha (csr)
    """
    H = compute_H(X, kernel=kernel)
    # Initial sparse graph from H to define Laplacian for heat
...
def plot_adj_matrices(Ws: List[np.ndarray], titles: List[str], out_path: str, vmin: float = 0.0, vmax: float | None = None):
    m = len(Ws)
    ncols = min(m, 3)
...
def plot_adj_differences(W1: np.ndarray, W2: np.ndarray, name1: str, name2: str, out_path: str):
    diff = W1 - W2
...
def heat_diffuse_from_seeds(A: csr_matrix, seeds: List[int], t: float, rank: int = 128) -> np.ndarray:
    """Return an (n, len(seeds)) array of heat diffusion scores from each seed."""
    n = A.shape[0]
...
def scatter_diffusion(X2d: np.ndarray, S: np.ndarray, seeds: List[int], title: str, out_path: str):
    """Plot diffusion intensity as color (average over seeds) on 2D scatter."""
...
class AblationConfig:
    dataset: str = "moons"  # moons | circles | swiss
    n: int = 800
    noise: float = 0.1
    k: int = 10
    alpha: float = 0.5
    ...
    seed: int = 42
    # visualization
    max_heatmap_n: int = 400  # if n exceeds, subsample for heatmaps to keep images readable
    n_seeds: int = 3
...
def run_ablation_and_visualization(cfg: AblationConfig, outdir: str = "results") -> Dict:
    ...
    # Build graphs and intermediates
    ...
    # Optionally compute simple metrics when labels available (moons/circles)
    ...
    # Heatmaps (subsample if too large)
    ...
    # Pairwise differences among the top-k graphs
    ...
    # Diffusion extent visualization (only for 2D scatter friendly datasets)
    seeds = np.random.RandomState(cfg.seed).choice(len(X), size=min(cfg.n_seeds, len(X)), replace=False).tolist()
    if cfg.dataset in {"moons", "circles"}:
        X2d = X
    else:
        # swiss: simple projection to first two dimensions for overview
        X2d = X[:, :2]
