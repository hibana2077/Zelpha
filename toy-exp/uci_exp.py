...
# Reuse training and graph utilities from existing experiments
from two_moons_circles_exp import (
    GraphConfig,
    TrainConfig,
    train_and_eval,
...
from swiss_roll_experiment import build_cosine_knn_graph, zelpha_graph
...
def load_ionosphere_csv(path: str) -> Tuple[np.ndarray, np.ndarray]:
    """Load Ionosphere CSV placed under the zelpha folder.

    Expected format: numeric features with final column as label 'g' or 'b'.
    Returns X (float64) and y (int) where g->1, b->0.
    """
    # Load as object then split
...
def aggregate_stats(values: List[float]) -> Dict[str, float]:
    ...
    return {"mean": float(np.nanmean(arr)), "std": float(np.nanstd(arr, ddof=1) if len(arr) > 1 else 0.0)}


def run_uci(cfg: UCIRunConfig, use_cuda: bool | None = None) -> Dict:
    """Run UCI classification with cosine vs Zelpha graphs across sweeps of (alpha, t).

    For each dataset and each (alpha, t), we run multiple seeds and report mean±std of
    test accuracy and macro-F1, along with paired t-test p-values vs cosine.
    """
    # Lazy import torch here to avoid dependency at import time
    import torch

    device = torch.device("cuda" if (use_cuda if use_cuda is not None else torch.cuda.is_available()) else "cpu")

    results: Dict[str, dict] = {}
    # ionosphere.csv is expected to be next to this script
    ...
    for ds_name in cfg.datasets:
        ...
        # Optional standardization applied consistently to graph + GCN and baselines
        ...
        for seed in cfg.seeds:
            ...
            # Baselines (raw features with internal standardization pipelines)
        ...
        # Aggregate and compute paired t-tests vs cosine
        ds_result = {
            "config": {
                "k": cfg.k,
                "alphas": cfg.alphas,
        ...
        for (alpha, t), rec in sweep_records.items():
            acc_vals = rec["acc"]
            f1_vals = rec["f1"]
            # Paired t-test vs cosine across seeds
            try:
...
def main():
    ...
    parser.add_argument("--datasets", type=str, nargs="*", default=["digits", "wine", "breast_cancer", "ionosphere"],
                        help="Subset of datasets to run: digits wine breast_cancer ionosphere")
